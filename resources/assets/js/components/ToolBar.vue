<template>
  <v-toolbar fixed app dark clipped-left class="main-toolbar" height="50px">
    ...
    <template v-if="authenticated">
      <v-btn flat @click="changeTipe" class="text-capitalize">{{ $store.state.info.tipe }}</v-btn>
      <v-menu origin="center center" offset-y :nudge-bottom="10" transition="scale-transition">        
        <v-btn flat slot="activator" class="text-capitalize" ><v-icon left>person</v-icon>{{ user.name }}</v-btn>
        <v-list class="pa-0">
          <v-list-tile v-for="(item,index) in account_items" @click="accountMenuItemClicked(item.action)" ripple="ripple" :disabled="item.disabled" :target="item.target" rel="noopener" :key="index">
            ...
            <v-list-tile-content>
              <v-list-tile-title class="text-capitalize">{{ item.title }}</v-list-tile-title>
            </v-list-tile-content>
          </v-list-tile>
        </v-list>
      </v-menu>
...
export default {
  ...
  methods: {
    changeTipe(){
      this.$store.commit('SET_TIPE')

      if(this.$store.state.info.tipe == 'reguler'){
        this.$router.push({ name: "home" });
      }else{
        this.$router.push({ name: "home.ktp" });
      }
